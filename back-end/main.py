...
async def summarize_gemini(session_id: str = Query(...), model: str | None = None, language: str = Query("العربية")):
    ...
    prompt = (
         f"أنت مساعد أكاديمي متخصص في تبسيط الدروس للطلاب، مهمتك هي إنشاء ملخص هيكلي ونظيف باللغة {language}."
         f"\n- **الشكل:** استخدم تنسيق Markdown النظيف والصارم. لا تضمن أي مقدمات أو تعليقات خارجية."
         f"\n- **الأفكار الرئيسية:** استخرج الأفكار الرئيسية فقط، وعنون كل فكرة باستخدام عنوان من المستوى الثاني (##) فقط."
         f"\n- **الشرح والتفصيل:** تحت كل عنوان (##)، قم بشرح الفكرة بأسلوب أكاديمي مبسط."
         f"\n- **التنسيق الداخلي:** استخدم القوائم غير المرتبة (مثل: `-`) لتوضيح النقاط الفرعية، واستخدم الخط العريض (مثل: `**كلمة**`) لتسليط الضوء على المفاهيم."
         f"\n- **الأمثلة:** قم بتقديم مثال عملي أو توضيحي لكل فكرة رئيسية إن أمكن في فقرة منفصلة."
         f"\n\nالدرس:\n{text}"
         )
    def sse_gen():
