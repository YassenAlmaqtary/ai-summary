...
async def summarize_gemini(session_id: str = Query(...), model: str | None = None, language: str = Query("العربية")):
    ...
    def build_prompt(text: str) -> str:
     return (
        f"أنت خبير في تلخيص الدروس الأكاديمية. مهمتك إنشاء ملخص احترافي للطلاب بطريقة جذابة وسهلة القراءة.\n\n"
        f"**التعليمات:**\n"
        f"1. استخدم Markdown بشكل كامل: عناوين رئيسية (#، ##)، فقرات منفصلة، قوائم نقطية ومرقمة، إبراز الكلمات المهمة (**bold**).\n"
        f"2. لكل فكرة رئيسية، ضع عنوانًا واضحًا على مستوى ##.\n"
        f"3. بعد كل عنوان، ضع فقرة قصيرة 2-4 جمل تشرح الفكرة بشكل مبسط.\n"
        f"4. أضف قائمة نقطية للنقاط الفرعية أو الأمثلة العملية.\n"
        f"5. اترك سطر فارغ بين كل فقرة وقائمة لتسهيل القراءة.\n"
        f"6. ضع مثالًا أو تطبيقًا عمليًا إذا كان موجودًا.\n"
        f"7. أضف قسم أخير بعنوان ## أسئلة وأجوبة يحتوي 3 إلى 7 أسئلة مع الإجابات، كل سؤال في سطر منفصل.\n"
        f"8. لا تكتب أي مقدمات أو تعليقات خارجية.\n"
        f"9. اجعل الملخص جذابًا، منظمًا، وسهل المراجعة، بحيث يبدو كدروس على موقع تعليمي عالمي.\n\n"
        f"**النص الأصلي:**\n{text}"
    )
